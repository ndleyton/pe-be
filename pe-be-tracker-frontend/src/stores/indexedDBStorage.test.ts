...
describe('createIndexedDBStorage', () => {
  ...
  it('falls back to localStorage when IndexedDB is unavailable', async () => {
    ...
    vi.spyOn(IndexedDBStorage.prototype, 'setItem').mockImplementation(async () => {
      throw new Error('IndexedDB unavailable');
    });
    vi.spyOn(IndexedDBStorage.prototype, 'removeItem').mockImplementation(async () => {
      throw new Error('IndexedDB unavailable');
    });
  ...
  it('falls back to localStorage when IndexedDB operations fail after readiness', async () => {
    ...
    vi.spyOn(IndexedDBStorage.prototype, 'removeItem').mockImplementation(async (key) => {
      if (key === '__idb_test__') {
        return;
      }
    ...
    vi.spyOn(IndexedDBStorage.prototype, 'getItem').mockImplementation(async () => {
