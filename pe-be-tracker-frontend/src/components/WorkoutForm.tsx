...
const WorkoutForm: React.FC<WorkoutFormProps> = ({ onWorkoutCreated }) => {
  ...
  const datePrefix = useMemo(() => new Date().toISOString().slice(0, 10), []);

  useEffect(() => {
    if (workoutTypeId && (!formState.dirtyFields.name || nameField === datePrefix)) {
      setValue('name', `${datePrefix} - ${workoutTypeId}`);
    }
  }, [workoutTypeId, datePrefix, formState.dirtyFields.name, nameField, setValue]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mb-6 border border-gray-700 p-6 rounded-lg bg-gray-900 text-gray-100 shadow-lg w-full max-w-md mx-auto">
      ...
      <div className="mb-4">
        ...
        {formState.errors.workout_type_id && <div className="text-red-400 text-sm">{formState.errors.workout_type_id.message}</div>}
      </div>
      <button
        type="submit"
