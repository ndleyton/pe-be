...
const AnatomicalImage = ({ muscleGroupSummary }: AnatomicalImageProps) => {
  ...
  const processedSvgContent = useMemo(() => {
    ...
    Object.values(MUSCLE_GROUP_MAPPING)
      .flat()
      .forEach((id) => {
    ...
    const maxSets = muscleGroupSummary.reduce(
      (max, group) => Math.max(max, group.setCount),
      0,
    );
  ...
  return (
    <div className="anatomical-image-container h-full w-full">
      <div
