...
describe("ExerciseRow", () => {
  ...
  it("clears committed drafts on Enter so subsequent stepper clicks increment display without repeating stale draft", async () => {
    const user = userEvent.setup();
    const { container } = render(<ExerciseRow {...defaultProps} />);
    ...
    const repsInput = repsInputs[0];
    const plusButtons = screen.getAllByRole("button", { name: /increase reps/i });
    ...
    await user.clear(repsInput);
    await user.type(repsInput, "12{enter}");

    expect(repsInput).toHaveValue("12");

    // Click plus: model and display must advance to 13 (not remain 12)
    await user.click(plusButtons[0]);
    expect(repsInput).toHaveValue("13");

    // Click plus again: model and display must advance to 14 (not repeat 13)
    await user.click(plusButtons[0]);
    expect(repsInput).toHaveValue("14");
  });

  it("commits typed draft and advances when clicking stepper directly without Enter or blur", async () => {
    const user = userEvent.setup();
    const { container } = render(<ExerciseRow {...defaultProps} />);
    ...
    // Type 12 without Enter or blur
    await user.clear(repsInput);
    await user.type(repsInput, "12");
    expect(repsInput).toHaveValue("12");

    // Click plus directly: commits draft (12) + 1 = 13 and clears draft
    await user.click(plusButtons[0]);
    expect(repsInput).toHaveValue("13");

    // Next click advances to 14
    await user.click(plusButtons[0]);
    expect(repsInput).toHaveValue("14");
  });
});
