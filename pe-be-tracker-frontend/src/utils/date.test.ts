...
describe("Date Utilities", () => {
  describe("toUTCISOString", () => {
    ...
    it("handles HTML datetime-local format without seconds", () => {
      ...
      expect(toUTCISOString(datetimeLocal)).toBe(
    ...
    it("handles edge case timezone formats", () => {
      expect(toUTCISOString("2024-01-01T12:00:00+0530")).toBe(
        "2024-01-01T06:30:00.000Z",
      );
      expect(toUTCISOString("2024-01-01T12:00:00-08:00")).toBe(
  ...
  describe("parseWorkoutDuration", () => {
    ...
    it("handles invalid date strings", () => {
      expect(
        parseWorkoutDuration("invalid-date", "2024-01-01T11:00:00Z"),
      ).toEqual({
        durationMs: 0,
      ...
      expect(
        parseWorkoutDuration("2024-01-01T10:00:00Z", "invalid-date"),
      ).toEqual({
        durationMs: 0,
        durationText: "Invalid duration",
      });
  ...
  describe("Formatter Memoization", () => {
    ...
    it("creates different formatters for different options", () => {
      ...
      const withTimezone = formatDisplayDate(timestamp, {
        includeTimezone: true,
      });
