...
export const ErrorTestComponent: React.FC<ErrorTestComponentProps> = ({ className }) => {
  ...
  const triggerPromiseRejection = () => {
    ...
    Promise.reject(new Error('Promise rejection - this will not be caught by error boundaries'));
  };
  ...
  const triggerMapError = () => {
    ...
    try {
      ...
      console.error('Map error (caught in try-catch, not by error boundary):', error);
    }
  };
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>
          Development tool for testing error boundaries. Only visible in development mode.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="text-xs text-muted-foreground mt-4 p-2 bg-muted rounded">
          ...
          They do NOT catch errors in event handlers, async code, or during server-side rendering.
        </div>
      </CardContent>
    </Card>
  );
