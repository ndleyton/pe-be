...
Use a string CHECK constraint on each table: `side IS NULL OR side IN ('left', 'right', 'both')`. Do not introduce a PostgreSQL enum for this small evolving attribute. Existing records remain NULL during the schema migration; no automatic name- or note-based backfill is performed. A standalone side index is not required initially: statistics already scope by exercise type and user, so add an index only if query plans justify it.
...
> **TODO — reviewed note-based side backfill:** Add a standalone, manually invoked script that proposes `exercise_sets.side` values from explicit side markers in historical set notes. It must default to dry-run mode, use a documented case-insensitive mapping for unambiguous markers such as `left`, `right`, `both`, `L`, and `R`, and emit the set ID, original note, proposed side, and matched rule for review. Ambiguous or conflicting notes must remain NULL and be reported separately. Apply mode must require an explicit confirmation flag, update only rows whose side is still NULL, preserve notes unchanged, be idempotent, process rows in bounded batches, and write an auditable summary of updated, skipped, and ambiguous counts. Add fixture-based tests for false positives (including words containing `left` or `right`), conflicting markers, punctuation and casing, already-tagged rows, and retry behavior. This script is a follow-up data-cleanup tool, not part of the schema migration or automatic deployment.
