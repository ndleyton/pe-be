...
class Settings(BaseSettings):
    ...
    GOOGLE_CLIENT_ID: str = Field("test-google-client-id", validation_alias="GOOGLE_CLIENT_ID")
    ...
    GOOGLE_REDIRECT_URI: str = Field(
        "http://localhost:8000/api/v1/auth/google/callback", validation_alias="GOOGLE_REDIRECT_URI"
    )
    ...
    FRONTEND_POST_LOGIN_PATH: str = Field("/dashboard", validation_alias="FRONTEND_POST_LOGIN_PATH")
    ...
    LANGFUSE_HOST: str = Field("https://us.cloud.langfuse.com", validation_alias="LANGFUSE_HOST")

    # Import database configuration (for external data sources)
    IMPORT_DATABASE_HOST: str = Field("localhost", validation_alias="IMPORT_DATABASE_HOST")
    ...
    IMPORT_DATABASE_USER: str = Field("postgres", validation_alias="IMPORT_DATABASE_USER")
    IMPORT_DATABASE_PASSWORD: str = Field("postgres", validation_alias="IMPORT_DATABASE_PASSWORD")

    # Image configuration
    IMAGE_URL_PREFIX: str = Field(
        "", validation_alias="IMAGE_URL_PREFIX", description="URL prefix for exercise images"
    )
    ...
    COOKIE_SECURE: bool = Field(
        False, validation_alias="COOKIE_SECURE", description="Enable secure cookies for HTTPS"
    )
