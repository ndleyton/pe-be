...
class RecipeService:
    ...
    async def get_user_recipes(self, session: AsyncSession, user_id: int) -> List[RecipeRead]:
        """Get all recipes for a user"""
        recipes = await crud.get_user_recipes(session, user_id)
        return [RecipeRead.from_orm(recipe) for recipe in recipes]
    
    async def get_recipe(self, session: AsyncSession, recipe_id: int, user_id: int) -> Optional[RecipeRead]:
    ...
    async def create_recipe(self, session: AsyncSession, recipe_data: RecipeCreate, user_id: int) -> RecipeRead:
        """Create a new recipe"""
        recipe = await crud.create_recipe(session, recipe_data, user_id)
        return RecipeRead.from_orm(recipe)
    
    async def update_recipe(self, session: AsyncSession, recipe_id: int, recipe_data: RecipeUpdate, user_id: int) -> Optional[RecipeRead]:
    ...
    async def delete_recipe(self, session: AsyncSession, recipe_id: int, user_id: int) -> bool:
        """Delete a recipe"""
