...
class ExerciseSetService:
    ...
    @staticmethod
    async def get_exercise_set(session: AsyncSession, exercise_set_id: int, user_id: int) -> Optional[ExerciseSet]:
        ...
        if exercise_set.exercise.workout.owner_id != user_id:
            raise HTTPException(status_code=403, detail="Not authorized to access this exercise set")
        
        return exercise_set
    
    @staticmethod
    async def get_exercise_sets_for_exercise(session: AsyncSession, exercise_id: int, user_id: int) -> List[ExerciseSet]:
    ...
    @staticmethod
    async def create_new_exercise_set(session: AsyncSession, exercise_set_data: ExerciseSetCreate, user_id: int) -> ExerciseSet:
        """Create a new exercise set with ownership verification"""
        # Verify the exercise exists and belongs to the user
        exercise = await verify_exercise_ownership(session, exercise_set_data.exercise_id, user_id)
        if not exercise:
            raise HTTPException(status_code=404, detail="Exercise not found")
    ...
    @staticmethod
    async def remove_exercise_set(session: AsyncSession, exercise_set_id: int, user_id: int) -> bool:
