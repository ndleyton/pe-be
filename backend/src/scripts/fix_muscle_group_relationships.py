...
def normalize_database_url(database_url: str) -> str:
    """Convert async SQLAlchemy URLs into psycopg2-friendly URLs."""
...
def plan_updates(
    muscles: Iterable[MuscleRow],
) -> tuple[list[PlannedUpdate], list[MuscleRow]]:
...
def ensure_group_ids(
    cursor, desired_group_names: set[str], existing_group_ids: dict[str, int]
) -> dict[str, int]:
...
def apply_updates(
    cursor, updates: Iterable[PlannedUpdate], group_name_to_id: dict[str, int]
) -> int:
    now = datetime.now(timezone.utc)
    applied = 0
