...
async def test_get_last_exercise_performance_no_exercise(mock_get_types, chat_service_with_db):
...
async def test_get_last_exercise_performance_no_stats(mock_stats, mock_types, chat_service_with_db):
...
async def test_get_last_workout_summary_exception(mock_get_latest, chat_service_with_db):
...
@pytest.mark.asyncio
@patch("src.chat.service.get_latest_workout_for_user")
async def test_get_last_workout_summary_no_workout(mock_get_latest, chat_service_with_db):
...
@patch("src.chat.service.get_latest_workout_for_user")
async def test_get_last_workout_summary_exercises_exception(mock_get_latest, mock_get_exercises, chat_service_with_db):
...
@patch("src.chat.service.get_exercises_for_workout")
@patch("src.chat.service.get_latest_workout_for_user")
async def test_get_last_workout_summary_generate_exception(mock_get_latest, mock_get_exercises, chat_service_with_db):
    mock_workout = MagicMock(id=1, name="Test")
...
@patch("src.chat.service.get_exercises_for_workout")
@patch("src.chat.service.get_latest_workout_for_user")
async def test_get_last_workout_summary_no_exercises(mock_get_latest, mock_get_exercises, chat_service_with_db):
...
@patch("src.chat.service.get_exercises_for_workout")
@patch("src.chat.service.get_workout_by_date")
async def test_get_workout_summary_by_date_no_exercises(mock_get_workout, mock_get_exercises, chat_service_with_db):
    mock_workout = MagicMock(id=1, name="Test Workout")
...
async def test_generate_response_empty_final_message_has_generation(chat_service_no_db):
    ...
    with patch("src.chat.service.ChatService._get_llm_client", return_value=mock_llm), \
...
async def test_chat_service_initialization_with_clients():
    ...
    with patch("src.chat.service.settings.LANGFUSE_PUBLIC_KEY", "pub_key"), \
...
async def test_generate_response_without_generation_end(mock_get_llm, chat_service_no_db):
...
async def test_parse_workout_and_save_tool_wrapper(chat_service_with_db):
    ...
    with patch("src.workouts.service.WorkoutService.create_workout_from_parsed") as mock_create:
...
async def test_get_workout_summary_by_date_intensity_display_paths(chat_service_no_db):
    ...
    with patch("src.chat.service.get_workout_by_date", return_value=mock_workout), \
         patch("src.chat.service.get_exercises_for_workout", return_value=[mock_exercise]):
        chat_service_no_db.session = AsyncMock() # Required to not trip the no session check
...
async def test_get_last_workout_summary_intensity_display_paths(chat_service_no_db):
    ...
    with patch("src.chat.service.get_latest_workout_for_user", return_value=mock_workout), \
         patch("src.chat.service.get_exercises_for_workout", return_value=[mock_exercise]):
