...
async def test_create_workout_from_recipe_success(db_session: AsyncSession):
    ...
    et = ExerciseType(name="Svc Exercise", description="x", default_intensity_unit=iu.id)
    db_session.add(et)
    await db_session.flush()
    ...
    db_session.add_all(
        [
            SetTemplate(reps=12, intensity=30.0, intensity_unit_id=iu.id, exercise_template_id=etmpl.id),
            SetTemplate(reps=10, intensity=35.0, intensity_unit_id=iu.id, exercise_template_id=etmpl.id),
        ]
    )
    ...
    res = await db_session.execute(
        select(func.count()).select_from(Exercise).where(Exercise.workout_id == workout.id)
    )
    exercise_count = res.scalar()
    assert exercise_count == 1

    res2 = await db_session.execute(select(Exercise.id).where(Exercise.workout_id == workout.id))
    ex_id = res2.scalar_one()

    from src.exercise_sets.models import ExerciseSet

    res3 = await db_session.execute(
        select(func.count()).select_from(ExerciseSet).where(ExerciseSet.exercise_id == ex_id)
    )
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_create_workout_from_recipe_not_accessible_raises(db_session: AsyncSession):
    ...
    u1 = User(email="a@example.com", hashed_password="x", is_active=True, is_superuser=False, is_verified=True)
    u2 = User(email="b@example.com", hashed_password="x", is_active=True, is_superuser=False, is_verified=True)
