...
async def test_handle_chat_error(mock_chat_service, override_get_current_user):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.post("/chat", json={"messages": [{"role": "user", "content": "hi"}], "conversation_id": None})
...
@pytest.mark.asyncio
@patch("src.chat.router.get_conversation_by_id")
async def test_get_conversation_loads_messages_error(mock_get, override_get_current_user):
    mock_conv = MagicMock()
    mock_conv.id = 1
...
@pytest.mark.asyncio
@patch("src.chat.router.update_conversation")
async def test_update_conversation_not_found_bubbles(mock_update, override_get_current_user):
...
@pytest.mark.asyncio
@patch("src.chat.router.delete_conversation")
async def test_delete_conversation_not_found_handled(mock_delete, override_get_current_user):
