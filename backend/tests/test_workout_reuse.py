...
def add_exercise_payload():
    return AddExerciseRequest(
        ...
        initial_set=ExerciseSetInput(
            reps=10, intensity=100, intensity_unit_id=1, rest_time_seconds=60
        ),
    )


@pytest.mark.asyncio
async def test_reuse_recent_unfinished_workout(
    ...
    recent_start = datetime.now(timezone.utc) - timedelta(
        hours=WORKOUT_REUSE_WINDOW_HOURS - 1
    )
...
@pytest.mark.asyncio
async def test_create_new_if_latest_is_finished(
    mock_session, user_id, add_exercise_payload
):
    ...
    new_workout = Workout(
        id=101, owner_id=user_id, start_time=datetime.now(timezone.utc)
    )

    with (
        patch(
            "src.workouts.service.get_latest_workout_for_user",
            return_value=finished_workout,
        ),
        patch(
...
@pytest.mark.asyncio
async def test_create_new_if_latest_is_too_old(
    mock_session, user_id, add_exercise_payload
):
    ...
    old_workout = Workout(id=100, owner_id=user_id, start_time=old_start, end_time=None)
    new_workout = Workout(
        id=101, owner_id=user_id, start_time=datetime.now(timezone.utc)
    )
    ...
    with (
        patch(
            "src.workouts.service.get_latest_workout_for_user", return_value=old_workout
        ),
        patch(
            "src.workouts.service.create_workout", return_value=new_workout
        ) as mock_create_workout,
...
@pytest.mark.asyncio
async def test_create_new_if_no_previous_workout(
    mock_session, user_id, add_exercise_payload
):
    ...
    with (
        patch("src.workouts.service.get_latest_workout_for_user", return_value=None),
        patch(
            "src.workouts.service.create_workout", return_value=new_workout
        ) as mock_create_workout,
        patch(
