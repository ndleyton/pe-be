        ...
        [
            {
                ...
                "description": "Steady-state cardio exercises at 50-65% of max heart rate",
                "created_at": now,
                "updated_at": now,
            },
            {
                "id": 2,
                "name": "HIIT",
                "description": "High-Intensity Interval Training alternating between intense exercise and rest periods",
                "created_at": now,
                "updated_at": now,
            },
            {
                "id": 3,
                "name": "Sports",
                "description": "Athletic activities and sports-specific training",
                "created_at": now,
                "updated_at": now,
            },
            {
                ...
                "description": "Progressive resistance training focusing on building strength and muscle",
                "created_at": now,
                "updated_at": now,
            },
            {
                "id": 5,
                "name": "Mobility",
                "description": "Exercises focusing on flexibility, range of motion, and joint health",
                "created_at": now,
                "updated_at": now,
            },
        ],
...
def downgrade() -> None:
    ...
    table_configs = [
        (
            ...
            [1, 2, 3],
            [1, 2, 3, 4, 5, 6, 7, 8],
        ),  # (table, exercise_type_ids, muscle_ids)
    ...
    for table_name, ids, muscle_ids in table_configs:
        if table_name in inspector.get_table_names():
            if table_name == "exercise_types_muscles":
                ...
                # Regular tables
                connection.execute(
                    sa.text(f"DELETE FROM {table_name} WHERE id = ANY(:ids)"), ids=ids
                )
