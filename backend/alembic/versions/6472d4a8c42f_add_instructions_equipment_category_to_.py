...
def upgrade() -> None:
    ...
    if "instructions" not in columns:
        op.add_column(
            "exercise_types", sa.Column("instructions", sa.Text(), nullable=True)
        )

    if "equipment" not in columns:
        op.add_column(
            "exercise_types", sa.Column("equipment", sa.String(), nullable=True)
        )

    if "category" not in columns:
        op.add_column(
            "exercise_types", sa.Column("category", sa.String(), nullable=True)
        )
