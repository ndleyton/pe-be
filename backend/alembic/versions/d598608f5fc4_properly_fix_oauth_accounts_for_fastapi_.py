...
def upgrade() -> None:
    ...
    if not constraint_result.fetchone():
        op.create_unique_constraint(
            "uq_oauth_accounts_oauth_name_account_id",
            "oauth_accounts",
    ...
    user_id_result = connection.execute(
        sa.text(
            "SELECT column_name FROM information_schema.columns WHERE table_name = 'oauth_accounts' AND column_name = 'user_id'"
        )
    )
    if not user_id_result.fetchone():
        # Add user_id column if it doesn't exist
        op.add_column(
            "oauth_accounts", sa.Column("user_id", sa.Integer(), nullable=False)
        )
...
def downgrade() -> None:
    ...
    if fk_result.fetchone():
        op.drop_constraint(
            "fk_oauth_accounts_user_id", "oauth_accounts", type_="foreignkey"
        )
    ...
    if constraint_result.fetchone():
        op.drop_constraint(
            "uq_oauth_accounts_oauth_name_account_id", "oauth_accounts", type_="unique"
        )
    ...
    if current_pk and current_pk[0] == ["id"]:
        ...
        op.create_primary_key(
            "oauth_accounts_pkey", "oauth_accounts", ["user_id", "oauth_name"]
        )
